  ...
  return (
    ...
    >
      <div className="h-12 border-b px-3 flex items-center justify-between">
        <div className="text-sm font-semibold tracking-tight">{t("chat.chats")}</div>
        <Button
          variant="secondary"
          onClick={() => {
            startNewChat();
      ...
      <Dialog open={Boolean(prompt)} onOpenChange={(open) => !open && openPrompt(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{promptTitle(prompt, t)}</DialogTitle>
            <DialogDescription>{t("chat.promptDescription")}</DialogDescription>
          </DialogHeader>
          <Input
            autoFocus
            value={name}
      ...
      <Dialog open={Boolean(deleteDialog)} onOpenChange={(open) => !open && setDeleteDialog(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {deleteDialog ? t("chat.deleteConfirm", { name: deleteDialog.chatName }) : ""}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
...
function promptTitle(prompt: PromptMode, t: (key: string) => string) {
  if (!prompt) return "";
  if (prompt.kind === "createChat") return t("chat.createChat");
  if (prompt.kind === "renameChat") return t("chat.renameChat");
  return "";
}
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" className="h-8 w-8" title={t("chat.chatActions")} disabled={disabled}>
          <MoreVertical className="h-4 w-4" />
          <span className="sr-only">{t("chat.chatMenu")}</span>
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end">
        <DropdownMenuItem onSelect={() => onRename()}>{t("chat.rename")}</DropdownMenuItem>
        <DropdownMenuSeparator />
