  ...
  return (
    <div className="h-12 border-b bg-card/60 backdrop-blur supports-[backdrop-filter]:bg-card/50 px-4 flex items-center justify-between gap-3">
      <div className="flex min-w-0 items-center gap-2">
        ...
        <div className="truncate text-sm font-semibold tracking-tight">{chat?.name ?? "No chat"}</div>
      </div>
      <Button variant="outline" className='lg:hidden' onClick={startNewChat} title="Start a new chat" disabled={isLoadingChats}>
        New chat
      </Button>
    </div>
  );
